...
async function checkData() {
  try {
    ...
    // 关键检查：HR company_id 与工�?company_id 是否匹配
    console.log('\n========================================');
    console.log('  关键关联检�?);
    console.log('========================================\n');
    ...
    if (hrUser?.company_id) {
      ...
      console.log(`HR 公司的项�? ${matchingProjects.length} 个`);
      ...
      for (const w of workLogs) {
        ...
        if (project) {
          ...
          console.log(`工时 ${w._id} 的项目属�?HR 公司: ${belongsToHRCompany}`);
        }
      }
    } else {
