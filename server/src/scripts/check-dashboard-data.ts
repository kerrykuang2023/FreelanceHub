...
async function checkDashboardData() {
  try {
    ...
    console.log(`Freelancer 的申�? ${freelancerApps.length}`);
    freelancerApps.forEach(app => {
      console.log(`  - 申请ID: ${app._id}, 状�? ${app.status}, 职位ID: ${app.job_post_id}`);
    });
    ...
    hrJobs.forEach(job => {
      console.log(`  - 职位ID: ${job._id}, 标题: ${job.job_title}, 状�? ${job.status}, company_id: ${job.company_id}`);
    });
    ...
    if (acceptedApps.length > 0) {
      ...
      console.log(`Freelancer 进行中的项目 (通过已接受申�?: ${projects.length}`);
      projects.forEach(p => {
    ...
    console.log(`HR 收到的申�? ${hrApplications.length}`);
    hrApplications.forEach(app => {
      console.log(`  - 申请ID: ${app._id}, 状�? ${app.status}`);
    });
    ...
    if (freelancerApps.length === 0) {
      ...
      console.log('�?Freelancer 有申请记�?);
    }

    if (acceptedApps.length === 0) {
      console.log('�?Freelancer 没有已接受的申请');
    } else {
      console.log('�?Freelancer 有已接受的申�?);
    }
