...
async function checkApprovalTestData() {
  try {
    ...
    console.log(`审批测试公司的工时记录: ${workLogs.length} 条`);
    for (const w of workLogs) {
      console.log(`  - ${w._id} | 状态: ${w.status} | 描述: ${w.work_description}`);
    }
    ...
    console.log(`\n审批测试公司的发票记录: ${invoices.length} 条`);
    for (const i of invoices) {
      console.log(`  - ${i._id} | 状态: ${i.status} | 描述: ${i.description}`);
    }
