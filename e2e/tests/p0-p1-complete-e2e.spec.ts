...
test.describe.serial('Complete P0/P1 Features E2E Test', () => {
  let page: Page;
  let context: BrowserContext;
  
  const testUsers = {
    freelancer: { email: 'freelancer@test.com', password: 'Test1234!' },
  ...
  test('AUTH-002-ACTION: Login as Admin', async () => {
    await page.goto(`${BASE_URL}/login`);
    await page.waitForLoadState('networkidle');
    ...
    await page.waitForURL(/\/(dashboard|admin|profile|jobs|\/$)/, { timeout: 15000 }).catch(() => {
      console.log('Login redirect may have failed, checking current URL');
    });

    await page.screenshot({ path: 'e2e-test-results/auth-002-login-result.png' });

    const currentUrl = page.url();
    if (currentUrl.includes('/login')) {
      logResult({
        featureId: 'AUTH-002-ACTION',
        featureName: '用户登录',
        status: 'fail',
        message: '登录失败，仍在登录页面'
      });
